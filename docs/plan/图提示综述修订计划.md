...
- **已正式完成的检查点：** `CP0`、`CP1`、`CP3`（部分）、`CP4`（表格层面）
- **当前实际进度：**
  ...
  - `tex/3.Preliminaries.tex`：内容未见过时表述，可视为基本完成。✅
  - `tex/4.Pre-training.tex`：内容未见过时表述，可视为基本完成。✅
  - `tex/table/graph_prompt_summary.tex`：表格已大幅更新，已包含 RELIEF(KDD 2025)、DAGPrompT(WWW 2025)、Self-Pro(ECML PKDD 2024)、PSP(ECML PKDD 2024)、SUPT(arXiv 2024)、EdgePrompt(ICLR 2025)、UniPrompt(NeurIPS 2025)、ProNoG(KDD 2025)、GraphPrompter(ICDE 2025) 等近年工作。✅
  - `tex/5.tex`：**表格已更新，但 prose 段落仍主要引用旧版方法**，需要将近年工作（RELIEF、DAGPrompT、Self-Pro、PSP、SUPT、EdgePrompt、UniPrompt、ProNoG 等）的描述与新表格对应。
  - `tex/6.Applications.tex`：仍为旧版应用章节，未见 overflow 承接更新。
  - `tex/7.ProG.tex`：仍将 ProG 主要表述为统一工具库，尚未刷新为已发表 benchmark/toolkit 论文定位。
  - `tex/8.Discussion.tex`：仍为旧版挑战与展望内容，未见近年趋势补充。
- **已完成的文献准备范围：**
  - `docs/papers/近期图提示论文池.md` 已完成保留条目、初步去向标签、本地 PDF 可用性和 BibTeX 覆盖核查。
  - `tex/zotero.bib` 已核对覆盖全部 19 篇保留论文，本轮未发现缺失条目需要补录。
- **尚未完成的正文范围：**
  - `tex/5.tex` prose 部分：需与已更新的表格对齐，补充近年代表方法的描述段落。
  - `tex/6.Applications.tex`：仍为旧版应用章节，未见 overflow 承接更新。
...
- **最近一次基线验证：** `latexmk -xelatex ...` 已在 `2026-04-26` 成功执行。
- **Task 1 产出：** `docs/research/稿件基线记录.md`

## 检查点总览

- **CP0 基线冻结：** 已完成
- **CP1 近期论文池就绪：** 已完成
- **CP2 Taxonomy 映射锁定：** 未完成（但表格已更新，实际上可与CP4合并推进）
- **CP3 前言部分更新：** 基本完成（`tex/0-4.tex` 均已无过时表述），ProG 措辞在 `tex/7.ProG.tex` 中尚未统一
- **CP4 核心 Taxonomy 更新：** 表格已完成，prose 部分待更新
- **CP5 溢出内容补入：** 未完成
- **CP6 参考文献与资源同步：** 未完成
- **CP7 发布候选版构建：** 未完成
...
- Update or create: `docs/research/稿件基线记录.md`
...
**文件：**
- Create: `docs/papers/近期图提示论文池.md`
...
- [x] Preliminaries (tex/3.Preliminaries.tex) 和 Pre-training (tex/4.Pre-training.tex) 经核查无过时表述。
- [ ] 重新审查前言部分和综述定位中的所有 ProG 相关措辞，使 ProG 被一致表述为已发表的 benchmark/toolkit 论文，而不只是内部开发库。**（仍在 tex/7.ProG.tex 中未处理）**

**检查点 CP3**

- 退出标准：前言部分已去除陈旧论断，内部表述一致，与新的修订范围对齐，并采用正式的综述语气。
- **实际状态：** 前言四章（0-4）已基本完成，ProG 措辞尚未统一。
...
- [x] 更新摘要表格，使新增引用不仅出现在段落文字中，也能出现在结构化对比里。**（表格已更新，包含 RELIEF、DAGPrompT、Self-Pro、PSP、SUPT、EdgePrompt、UniPrompt、ProNoG 等）**
- [ ] 更新 `tex/5.tex` prose 部分，使其与表格中的新方法对齐。
...
- **实际状态：** 表格层面已完成，prose 部分仍待更新以匹配表格。
...
- [x] 先做 Task 1
- [x] 然后做 Task 2
- [ ] 在任何正文大改之前完成 Task 3（可与 Task 5 并行推进）
- [x] 在 Task 5 之前完成 Task 4（基本完成，ProG 措辞除外）
- [ ] Task 5 prose 部分：当前重点，需与表格对齐
...
**已完成：** Sections 1-4 基本完成，Section 5 表格已更新（包含 2024-2026 代表工作）

**进行中：** Section 5 prose 部分需要与表格对齐，补充近年代表方法的描述

**待启动：** Section 6 (Applications overflow)、Section 7 (ProG benchmark 定位)、Section 8 (Discussion 趋势补充)

**下一步优先：** 更新 `tex/5.tex` prose 段落，将表格中的新方法（RELIEF、DAGPrompT、Self-Pro、PSP、SUPT、EdgePrompt、UniPrompt、ProNoG 等）以连贯的叙事方式接入现有 taxonomy 结构
