...
In this survey, we reviewed over one hundred representative papers, with particular attention to conference, journal, and arXiv works released through early 2026. These references cover venues including but not limited to NeurIPS, SIGKDD, The Web Conference, ICLR, CIKM, ICML, IJCAI, EMNLP, SIGIR, ACL, AAAI, WSDM, and TKDE. Most of these venues are ranked as CCF A\footnote{\url{https://www.ccf.org.cn/Academic_Evaluation/By_category}} or CORE A*\footnote{\url{https://www.core.edu.au/conference-portal}}. We also include recent arXiv papers to reflect fast-moving developments that have not yet fully stabilized into journal or conference versions. Figure \ref{fig:venue_dis} presents the venue distribution of the currently collected corpus, and Figure \ref{fig:top10keys} summarizes the top 15 keywords appearing in paper titles. These statistics provide a high-level snapshot of the literature covered in this survey. 
...
The most directly related surveys to the present manuscript are the graph-prompt-centered reviews by \citet{wu2023survey}, \citet{long2024towards}, and \citet{fu2025graphprompting}. The survey of \citet{wu2023survey} captured the first wave of graph prompting studies and mainly summarized techniques, applications, and challenges when the method space was still relatively small. The later survey of \citet{long2024towards} expanded the discussion toward design principles, applications, benchmark issues, and open problems, with particular emphasis on prompt tokenization, task alignment, and prompt tuning strategies. More recently, \citet{fu2025graphprompting} organized the area around the ``pre-train and prompt'' workflow and classified methods from data-level, representation-level, and task-level perspectives, giving a more mechanism-oriented view of graph prompting.

Compared with these surveys, the present manuscript keeps a stronger focus on a unified decomposition of graph prompts into prompt tokens, token structures, insertion patterns, answering functions, and prompt tuning strategies, while also explicitly connecting graph prompting to pre-training strategies, multi-modal prompting, and graph domain adaptation within one taxonomy. In addition, this revised version updates the coverage through early 2026 and preserves an engineering-oriented perspective through the discussion of applications and tool support such as ProG. Broader surveys on graph foundation models, graph-LLM integration, or graph self-supervised learning \cite{liu2023graph, li2023survey, xie2023selfsupervised, yu2023selfsupervised} remain useful background references, but they are not the primary comparison targets here because graph prompting is only one part of their scope.
